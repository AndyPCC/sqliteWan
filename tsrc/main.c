...
int sqlite3_initialize(void){
  ...
  */
  ...
  if( !sqlite3GlobalConfig.isMallocInit ){
    rc = sqlite3MallocInit(); //分配内存系统, 这里因为只初始化一次，所以lock manager也放这里调用
    //TODO: pxw
    //lock_manager_init();
  }
...
void sqlite3RollbackAll(sqlite3 *db, int tripCode){
  ...
  assert( sqlite3_mutex_held(db->mutex) );
  sqlite3BeginBenignMalloc();
  //fprintf(stdout, "sqlite3RollbackAll\n");
  ...
  ** corruption reports in some cases.  */
  ...
  for(i=0; i<db->nDb; i++){
    ...
    if( p ){
      ...
      sqlite3BtreeRollback(p, tripCode, !schemaChange); //pxw
    }
  }
  sqlite3VtabRollback(db);
  sqlite3EndBenignMalloc();
...
#endif
  *ppDb = 0;
#ifndef SQLITE_OMIT_AUTOINIT //如果没有定义自动init，则执行初始化函数
  // TODO:PXW 
  //printf("lala\n");
  rc = sqlite3_initialize(); //包含初始化资源
  if( rc ) return rc;
#endif
