...
static int sqlite3Step(Vdbe *p){
  ...
  if( p->explain ){ //解释模式
    rc = sqlite3VdbeList(p); //通过callback函数执行每一句
  }else
#endif /* SQLITE_OMIT_EXPLAIN */
  {
    db->nVdbeExec++;
    rc = sqlite3VdbeExec(p);//非解释模式，直接执行语句
    db->nVdbeExec--;
  }
...
int sqlite3_step(sqlite3_stmt *pStmt){
  ...
  sqlite3_mutex_enter(db->mutex);
  v->doingRerun = 0;
  while( (rc = sqlite3Step(v))==SQLITE_SCHEMA  //如果调用sqlite3Step()失败并且尝试次数小于设定次数，重试
