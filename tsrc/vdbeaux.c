...
/* 如果sqlite编译时支持sharedcache并且是线程安全的，此函数会给所有相关的BtShared
** 结构体上锁。
...
int sqlite3VdbeList(
  ...
  ** p->pc 最终到达nRow，nRow = 主程序中的行数 + 触发的子程序的行数
  */
  nRow = p->nOp;
  if( bListSubprogs ){
  ...
  do{
    ...
    */
    if( bListSubprogs && pOp->p4type==P4_SUBPROGRAM ){
      ...
      if( j==nSub ){
        p->rc = sqlite3VdbeMemGrow(pSub, nByte, nSub!=0); //分配内存？
        if( p->rc!=SQLITE_OK ){
          rc = SQLITE_ERROR;
          break;
        }
  ...
  if( 0==sqlite3Strlen30(sqlite3BtreeGetFilename(db->aDb[0].pBt))
   || nTrans<=1
    ...
    for(i=0; rc==SQLITE_OK && i<db->nDb; i++){
      Btree *pBt = db->aDb[i].pBt;
      if( pBt ){
        rc = sqlite3BtreeCommitPhaseOne(pBt, 0); //Commit phase 1
      }
    }
    ...
    */
    for(i=0; rc==SQLITE_OK && i<db->nDb; i++){
      Btree *pBt = db->aDb[i].pBt;
      if( pBt ){
        rc = sqlite3BtreeCommitPhaseTwo(pBt, 0); //Commit phase 2
      }
    }
    if( rc==SQLITE_OK ){
...
int sqlite3VdbeHalt(Vdbe *p){
  ...
  //pxw:print
  //fprintf(stdout, "sqlite3VdbeHalt\n");
...
/* 把pMem中的数据拷贝到buf，serial_type是数据类型，返回拷贝长度
